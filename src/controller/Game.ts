...
export class Game {
  ...
  public constructor({field: {selector: fieldSelector}, sizeX, sizeY}: GameConfig) {
    this.ships = [];
    this.hits = [];
    this.fieldView = new Field(fieldSelector, this.onSelectCell.bind(this), sizeX, sizeY);

    this.ships.push(new Ship([{x: 0, y: 0}, {x: 0, y: 1}]))
